...
public class FrameBuildingStep extends ApplicationStepAdapter {
	...
	public void start(String[] args) {
		...
		DesignerFrame frame = null;
		frame = new DesignerFrame(ApplicationModel.INTERFACE_BUILDER);
		XmlFormModel.MAIN_FRAME = frame;
		
		Rectangle r = Preferences.getPreference("dialog", "frame",
		...
		if ( args.length > 0 ) {
			...
			try {
				...
				if ( args.length > 1 ) {
					XmlFormModel.CURRENT_DOCUMENT = args[ 1 ];
					frame.setTitle( 
							frame.getTitle() + " [" + XmlFormModel.CURRENT_DOCUMENT + "]" );
				}
