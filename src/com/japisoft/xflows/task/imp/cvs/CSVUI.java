...
import com.japisoft.xflows.task.xslt.XSLTParametersDialog;
...
*/
public class CSVUI extends AbstractTaskUI {
	...
	FileTextField tfSourcePath = new FileTextField(null, "csv");

	JLabel lblSourceFilter = new JLabel();

	JComboBox cbSourceFilter = XFlowsFactory.getSourceFilter("csv");
	...
	JComboBox cbTargetName = XFlowsFactory.getTargetName("xml");
	...
	public void preSetParams() {
		if ( !params.hasParamValue( SOURCEFILTER ) ) {
			params.setParam( SOURCEFILTER, "(.*).csv" );
		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnParameters) {
			CSVParametersDialog dialog = new CSVParametersDialog( params );
	...
	void jbInit() throws Exception {
		titledBorder1 = new TitledBorder(BorderFactory.createLineBorder(
				new Color(153, 153, 153), 2), "CSV Import");
		lblSource.setText("CSV Source path");
		this.setBorder(titledBorder1);
		this.setLayout(gridBagLayout1);
		tfSourcePath.setText("");
		lblSourceFilter.setText("CSV Source filter");
		...
		this.add(btnParameters, new GridBagConstraints(1, 2, 1, 2, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(
						14, 9, 28, 0), 50, 0));

		this.add(lblParams, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(
						19, 17, 0, 34), 0, 0));
