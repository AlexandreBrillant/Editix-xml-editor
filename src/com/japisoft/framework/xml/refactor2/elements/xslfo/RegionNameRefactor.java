...
*/
public class RegionNameRefactor extends AbstractRefactor {
	...
	public boolean process(FPNode node) {
		...
		if ( node.hasAttribute( "region-name" ) ) {
			oldValue = node.getAttribute( "region-name" );
			if ( oldValue.startsWith( "xsl-region" ) )
				return false;
			return true;
		}

		if ( node.hasAttribute( "flow-name" ) ) {
			oldValue = node.getAttribute( "flow-name" );
			if ( oldValue.startsWith( "xsl-region" ) )
				return false;			
			return true;
		}
	...
	public String getTitle(FPNode node) {
		return "The region name '" + oldValue + "'";
	}
	...
	public void startElement(
		...
		if ( atts != null && 
					atts.getLength() > 0 && 
						atts.getValue( "region-name" ) != null && 
					!"".equals( atts.getValue( "region-name" ) ) ) {
			atts = new AttributesProxy( atts, "region-name" );
		}

		if ( atts != null && 
				atts.getLength() > 0 && 
					atts.getValue( "flow-name" ) != null && 
						!"".equals( atts.getValue( "flow-name" ) ) ) {
			atts = new AttributesProxy( atts, "flow-name" );
		}
