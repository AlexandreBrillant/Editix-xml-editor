...
public class FastLabel extends JComponent implements MouseListener {
	...
	static Color FOREGROUND_COLOR = Color.BLACK;
	
	static {
		Color tmp = UIManager.getColor( "editix.error.foreground" );
		if ( tmp != null )
			ERROR_COLOR = tmp;
		tmp = UIManager.getColor( "label.foreground" );
		if ( tmp != null )
			FOREGROUND_COLOR = tmp;
		
		tmp = UIManager.getColor( "editix.label.foreground" );
		if ( tmp != null )
			FOREGROUND_COLOR = tmp;
	}
	...
	public void paintComponent( Graphics gc ) {
		...
		if ( text != null ) {
			if ( message ) 
				...
				gc.setColor( error ? ERROR_COLOR : FOREGROUND_COLOR );
