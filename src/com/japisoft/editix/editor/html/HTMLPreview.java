...
public class HTMLPreview extends JPanel implements DocumentListener, ActionListener {
	...
	HTMLPreview( IXMLPanel editor ) {
		this.editor = editor;
		editor.getMainContainer().getEditor().getDocument().addDocumentListener( this );
		autoRefresh = Preferences.getPreference( "HTMLEditor", "auto-refresh", false );
	...
	public void dispose() {
		editor.getMainContainer().getEditor().getDocument().removeDocumentListener( this );
		editor = null;
	}
	...
	private String getHTMLPage() {
		String html = editor.getMainContainer().getText();
		if( editor.getMainContainer().getCurrentDocumentLocation() != null ) {
			int i = html.indexOf( "<head>" );
			if ( i > -1 ) {
				try {
					String uri = new File( 
						editor.getMainContainer().getCurrentDocumentLocation() ).getParentFile().toURL().toExternalForm();
					String baseInjector = "<base href='" + uri + "'>";
					html = html.substring( 0, i + 6 ) + baseInjector + html.substring( i + 6 );
				} catch( Exception exc ) {
				}
			}
		}
