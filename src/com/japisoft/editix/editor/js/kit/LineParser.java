...
public class LineParser {
	...
	private void initColorTable( String preferenceGroupe ) {
		...
		for ( int i = 0; i < syntax.getKeywords().length; i++ ) {
			mapOfColor.put( syntax.getKeywords()[ i ], kw );
		}
	...
	public Token[] getTokenForLine(Segment str, int line) {
		...
		for ( int i = 0; i < str.length(); i++ ) {
			...
			if ( !commentMode && stringMode == 0 ) {
				// Comment opening flag
				if ( syntax.isStartingLinesComment( c, oldc ) ) {
				...
				if ( syntax.isStartingLineComment( c, oldc ) ) { 
					sb.deleteCharAt( sb.length() - 1 );
					addToken( sb.toString() );
					resetBuffer();
					addToken( "//" );
			...
			if ( commentMode ) {				
				if ( lineCommentMode ) {
					...
					if ( syntax.isStoppingLinesComment( c, oldc ) ) {
