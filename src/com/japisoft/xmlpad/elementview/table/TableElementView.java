...
/**
 * Basic view using a <code>JTable</code>
 * <p>
	...
	class SimpleNodeModel implements TableModel {
		...
		public void update(FPNode node) {
			...
			if (node != null) {
				...
				try {
					String tmp = container.getDocument().getText( startingOffset, stoppingOffset - startingOffset );
					if ( tmp.endsWith( "/" ) )
						closed = true;
				...
				if (rowIndex == 1) {
					...
					if ((i = aValue.toString().indexOf(":")) > -1) {
						try {
							...
							Object[] __ = (Object[]) content.get(rowIndex);
							__[1] = name;

							Object[] ___ = (Object[]) content.get(0);
							___[1] = prefix;
