 ...
 * @see TagHelper
 */
public abstract class AbstractTagHelper extends AbstractHelper implements
		TagHelper {
	...
	public void setLocation(FPNode locatedNode, int offset) {
		...
		try {
			...
			if ((namespace != null) && (locatedNode != null)) {
				// Search for the first parent with the right namespace
				while (!namespace.equals(locatedNode.getNameSpaceURI())) {
	...
	public void fillList( FPNode node, DefaultListModel model ) {
		...
		for (int i = 0; i < tags.size(); i++) {
			Object __;
			model.addElement( __ = tags.get( i ) );
		}
