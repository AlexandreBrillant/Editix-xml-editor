...
*/
public class DefaultSystemHandler extends AbstractSystemHandler {
	...
	protected void installDescriptors( 
		...
		if ( document.isInProlog( offset )) {
			( ( AbstractDescriptor )addDescriptor( 
				new TagDescriptor( 
						addedString + "DOCTYPE tag SYSTEM \"\">", 
						null, 
						true, 
						true ) ) ).setComment( "DTD declaration part" );
		} else {
			...
			if ( !document.hasSchema() ) {
				...
				if ( currentNode != null ) {
					Document doc = currentNode.getDocument();
					if ( doc != null ) {
						List<String> l = doc.getOrderedTags();
			...
			( ( AbstractDescriptor )addDescriptor(
					new TagDescriptor( 
						...
						null, 
						true, 
						true ) ) ).setComment( "Section CDATA for text" );			
		}
