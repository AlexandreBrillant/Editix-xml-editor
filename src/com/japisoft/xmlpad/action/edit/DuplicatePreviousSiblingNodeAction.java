...
public class DuplicatePreviousSiblingNodeAction extends XMLAction {
	...
	public boolean notifyAction() {
		...
		if ( node == null ) {
			JOptionPane.showMessageDialog( container.getView(), "Can't find a previous sibling node", "Error", JOptionPane.ERROR_MESSAGE );
			return false;
		}

		int caret = container.getEditor().getCaretPosition();
		
		FPNode previousSiblingCandidate = null;
		
		for ( int i = 0; i < node.childCount(); i++ ) {
			FPNode child = node.childAt( i );
			if ( child.getStoppingOffset() < caret ) {
				previousSiblingCandidate = child;
			} else
				break;
		}
		
		if ( previousSiblingCandidate != null ) {

			Point n = CopyNodeAction.getNodeOffset( previousSiblingCandidate );
			try {
